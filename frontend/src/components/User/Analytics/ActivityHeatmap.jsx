...
export default function ActivityHeatmap({ habits = [], user = null }) {
    ...
    <div className="glass-panel inner-glow p-6 rounded-xl">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          {/* Legend */}
          <div className="flex items-center gap-2 text-[9px] tracking-wider uppercase text-on-surface-variant/40">
            <span>Less</span>
      ...
      <div className="overflow-x-auto pb-2">
        <div className="min-w-[800px] grid grid-flow-col grid-rows-7 gap-1">
          {days.map((day, index) => (
            <div
              key={index}
              className={`w-3 h-3 rounded-sm hover:scale-125 cursor-pointer transition-all duration-150 ${
                day.isCurrentYear
      ...
      <div className="flex justify-between mt-3 text-[9px] font-mono text-on-surface-variant/40 uppercase tracking-wider">
        {monthNames.filter((_, i) => i % 3 === 0).map((month, idx) => (
          <span key={idx}>{month} {selectedYear}</span>
        ))}
      </div>
    </div>
  );
