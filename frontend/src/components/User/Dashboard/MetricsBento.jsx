...
export default function MetricsBento({ habits  , completedCount, totalCount , user }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
      {/* Stat Card 1 */}
      <div className="glass-panel inner-glow p-4 sm:p-6 rounded-xl flex flex-col justify-between">
        ...
        <div className="mt-4">
          ...
          <span className="text-primary text-sm ml-2 font-bold block sm:inline">+2 this month</span>
        </div>
      </div>

      {/* Stat Card 2 */}
      <div className={`glass-panel inner-glow p-4 sm:p-6 rounded-xl flex flex-col justify-between border-b-2 transition-all duration-300 ${completedCount > 0 ? 'border-b-primary/50' : 'border-b-transparent'}`}>
        <div className="flex justify-between items-start">
